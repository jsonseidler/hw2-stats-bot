...
var get1 = function(options, eventVariables, gamertag, gamertagFormatted) {
  //get request
...
var get2 = function(options, eventVariables, gamertag, gamertagFormatted) {
  ...
  http.get(options, (res) => {
    ...
    res.on("end", () => {
      ...
      for(var i = 0; i < parsedData.RankedPlaylistStats.length; i++) {
        //find ranked 2 id
        if(parsedData.RankedPlaylistStats[i].PlaylistId == "379f9ee5-92ec-45d9-b5e5-9f30236cab00") {
          index = i;
          break;
        }
      }
      ...
      //check if user has not played games
      if(parsedData.RankedPlaylistStats[index] == undefined) {
        eventVariables.channel.send(util.format("<@!%s>, %s has not played any games of 2v2 War.", eventVariables.userID, gamertag));
        return(1);
      } else {
        //get and format time played
        var iso = parsedData.RankedPlaylistStats[index].TotalTimePlayed;
        ...
        const options = {
          hostname: "www.haloapi.com",
          path: util.format("/stats/hw2/playlist/379f9ee5-92ec-45d9-b5e5-9f30236cab00/rating?players=%s", gamertagFormatted),
          headers: {
            "Ocp-Apim-Subscription-Key": auth.key
          }
        };
...
var getSeason = function(options, eventVariables, gamertag, gamertagFormatted) {
  //get request
  http.get(options, (res) => {
    //check for valid gamertag
    if (res.statusCode == 404) {
...
var getAllTime = function(options, eventVariables, gamertag, gamertagFormatted) {
  //get request
  http.get(options, (res) => {
    //check for valid gamertag
    if (res.statusCode == 404) {
    ...
    res.on("end", () => {
      ...
      var messageFields = [];
      var highestDesignation = 0;
      for(var i = 0; i < parsedData.MatchmakingSummary.RankedPlaylistStats.length; i++) {
        //add data if user was placed
        if(parsedData.MatchmakingSummary.RankedPlaylistStats[i].HighestCsr != null) {
          //get name of playlist
          let playlistName;
          if(parsedData.MatchmakingSummary.RankedPlaylistStats[i].PlaylistId == "548d864e-8666-430e-9140-8dd2ad8fbfcd") {
            playlistName = "Ranked 1v1 X War";
          } else if(parsedData.MatchmakingSummary.RankedPlaylistStats[i].PlaylistId == "4a2cedcc-9098-4728-886f-60649896278d") {
            playlistName = "Ranked 3v3 X War";
          } else if(parsedData.MatchmakingSummary.RankedPlaylistStats[i].PlaylistId == "7c625f1c-4c66-4484-8cde-a261e3b4d104") {
            playlistName = "Ranked Xbox 2v2 War";
          ...
          //get rank
          var ranks = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Onyx", "Champion"];
          var rankNumber = parsedData.MatchmakingSummary.RankedPlaylistStats[i].HighestCsr.Designation;
          var rank = ranks[rankNumber-1];
          ...
          var tier = " ";
          if(rank == "Champion") {
            tier += parsedData.MatchmakingSummary.RankedPlaylistStats[i].HighestCsr.Rank;
          } else if(rank != "Onyx") {
            tier += parsedData.MatchmakingSummary.RankedPlaylistStats[i].HighestCsr.Tier;
          }

          //get csr
          var csr = parsedData.MatchmakingSummary.RankedPlaylistStats[i].HighestCsr.Raw;
      ...
      if(highestDesignation != 0) {
        ...
        eventVariables.channel.send({ embed: {
          author: {
            name: "All Time High Stats for " + gamertag
          },
          color: eventVariables.embedcolor,
          thumbnail: {
            url: "attachment://designation.png",
